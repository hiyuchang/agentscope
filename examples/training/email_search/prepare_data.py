...
def download_dataset(repo_id: str) -> Dataset:
    ...
    logging.info(
        f"Attempting to download dataset from Hugging Face Hub: {repo_id}"
    )
    ...
    dataset_obj = load_dataset(
        repo_id, features=expected_features, split="train"
    )
...
def generate_database(
    ...
    """
    logging.info(
        f"Starting database generation for repo '{repo_id}' at '{db_path}'"
    )
    ...
    logging.info("Please set the environment variable DEFAULT_EMAIL_DB_PATH to this path.")
